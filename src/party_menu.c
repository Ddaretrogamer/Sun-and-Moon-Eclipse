...
static void UpdateCurrentPartySelection(s8 *slotPtr, s8 movementDir)
{
    ...
    if (layout == PARTY_LAYOUT_DOUBLE
     || layout == PARTY_LAYOUT_MULTI_SHOWCASE
     || layout == PARTY_LAYOUT_MULTI_FULL_SHOWCASE
     || layout == PARTY_LAYOUT_MULTI_FULL_SHOWCASE_PARTNER)
...
static void UpdatePartySelectionSingleLayout(s8 *slotPtr, s8 movementDir)
{
    ...
    if (gPartyMenu.layout == PARTY_LAYOUT_SINGLE
     || gPartyMenu.layout == PARTY_LAYOUT_MULTI
     || gPartyMenu.layout == PARTY_LAYOUT_MULTI_FULL
     || gPartyMenu.layout == PARTY_LAYOUT_MULTI_FULL_PARTNER) // Custom equal layout
    {
        bool8 chooseHalf = sPartyMenuInternal->chooseHalf;
        ...
        case MENU_DIR_DOWN:
            ...
            {
                if ((*slotPtr + 2) < partyCount)
                {
                    *slotPtr += 2;
                }
                else if (chooseHalf)
                {
                    if (((*slotPtr & 1) == 0) || partyCount == 1)
            ...
            break;
        case MENU_DIR_RIGHT:
            if (*slotPtr == SLOT_CONFIRM && chooseHalf)
            {
                *slotPtr = SLOT_CANCEL;
            }
            else if (*slotPtr != SLOT_CANCEL
                  && *slotPtr < partyCount
                  && ((*slotPtr & 1) == 0)
                  && ((*slotPtr + 1) < partyCount))
            {
                (*slotPtr)++;
            }
            break;
        case MENU_DIR_LEFT:
            if (*slotPtr == SLOT_CANCEL && chooseHalf)
            {
                *slotPtr = SLOT_CONFIRM;
            }
            else if (*slotPtr < partyCount && ((*slotPtr & 1) == 1))
            {
                (*slotPtr)--;
            }
            break;
    ...
    else
    {
        ...
        case MENU_DIR_LEFT:
            ...
            else if (*slotPtr == SLOT_CONFIRM && sPartyMenuInternal->chooseHalf)
            {
                *slotPtr = SLOT_CANCEL;
            }
...
static void InitPartyMenuWindows(enum PartyMenuLayout layout)
{
    ...
    case PARTY_LAYOUT_MULTI_FULL_SHOWCASE_PARTNER:
        InitWindows(sShowcaseMultiPartyMenuWindowTemplate_Equal);
        break;
